...
export function useTweaks(defaults) {
  const [values, setValues] = useState(() => {
    const saved = localStorage.getItem('w2l_tweaks');
    if (saved) {
      try {
  ...
  const setTweak = useCallback((keyOrEdits, val) => {
    setValues((prev) => {
      ...
      localStorage.setItem('w2l_tweaks', JSON.stringify(updated));
      return updated;
    });
  }, []);
...
const __TwkCheck = ({ light }) => (
  <svg viewBox="0 0 14 14" aria-hidden="true" width="13" height="13" style={{ position: 'absolute', top: 6, left: 6 }}>
